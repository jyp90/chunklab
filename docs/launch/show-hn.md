...
Everything runs on 127.0.0.1; documents and API keys never leave the machine. The UI is FastAPI + htmx, no build step, `pip install chunkmatrix` and go.
...
- *`chunklab` on PyPI (unrelated project)*: similar goal (statistical comparison of chunkers), which is why ours ships as `chunkmatrix` on PyPI while keeping the `chunklab` CLI. Ours adds the browser labelling UI, hybrid retrieval axis, CI regression gate and agent CLI; theirs focuses on significance testing.
